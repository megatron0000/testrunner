...
export class Bundler {
  ...
  async _collectDeps(codeString, url) {
    ...
    while (match = codeString.match(/require\(\s*(?:'|")([^)'"]+?)(?:'|")\s*\)/)) {
      const [fullMatch, depName] = match;
      const newId = ++this.id;
      codeString = codeString.slice(0, match.index) +
        "require(" + newId + ")" +
      ...
      if (this.deps[depUrl]) { // already loaded or is being loaded
        continue;
      }
