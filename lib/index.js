...
import { runTest, runCode } from "./runner.js";
...
async function main() {
  ...
  //   stop = resolve;
  // });
  // const observable = runCode(rawUserCode, stopPromise);
  ...
  for (const node of testAST.body) {
    const testMessage = rawTestCode.slice(
      node.expression.arguments[0].start + 1, // remove initial "
      node.expression.arguments[0].end - 1 // remove final "
    );
